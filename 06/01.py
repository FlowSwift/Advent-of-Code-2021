...
def fish_calc2():
    with open("06/input.txt") as file_hnd:
        ...
        for j in range(days):
            ...
            fish = Counter({**fish,
                            **{i: tmp_fish[i + 1] if i != reset_phase else tmp_fish[reset_phase + 1] + tmp_fish[0] for i in range(8)}})  # push all the fishes a phase and add to `fish` overwriting existing values`. Also add 0 to reset phase
    return fish
...
def fish_calc3():
    with open("06/input.txt") as file_hnd:
        ...
        for j in range(days):
            tmp_fish = fish.copy()
            fish = Counter({born_phase: tmp_fish[0]} | {
                i: tmp_fish[i + 1] if i != reset_phase else tmp_fish[reset_phase + 1] + tmp_fish[0] for i in range(8)})  # similar approach using union
    return fish
